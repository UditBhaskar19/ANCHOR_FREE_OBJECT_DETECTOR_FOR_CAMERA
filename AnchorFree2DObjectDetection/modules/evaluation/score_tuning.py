...
def ROC_for_score_tuning(
    ...
    for _, score_thresh in enumerate(score_threshs):
        ...
        for i in range(image_start_idx, image_end_idx):
            ...
            condition = identify_prominent_objects(bboxes) & (clslabels != ignored_classId)
        ...
        if num_detections == 0 and num_false_positives == 0: precision = 1
        else: precision = num_detections / (num_detections + num_false_positives)
        f1_score = ( 2 * precision * recall ) / ( precision + recall )
        
        DETECTION_RATE_LIST.append(detection_rate)
        FP_RATE_PER_IMAGE_LIST.append(false_positives_per_image)
        SCORE_THRESH.append(score_thresh)
...
def display_performance(
    ...
    for i, (score, fp_rate, r, f1) \
        in enumerate(zip(score_thresh_list, fp_rate_per_image_list, det_rate_list, f1_list)):
        ss = f"{i}: ( Threshold: {score}, F1: {f1}, DET Rate: {r}, FP Rate: {fp_rate} )"
        print(ss)