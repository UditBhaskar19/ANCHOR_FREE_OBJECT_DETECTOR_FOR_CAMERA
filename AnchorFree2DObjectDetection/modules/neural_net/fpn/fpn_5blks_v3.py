# ---------------------------------------------------------------------------------------------------------------------------
# Author : Udit Bhaskar
# Description : Feature Pyramid Network (FPN) feature aggregator corrosponding to 5 levels of the feature pyramid
# ---------------------------------------------------------------------------------------------------------------------------
import torch
...
class FPN_block(nn.Module):
    ...
    def forward(self, x: Dict[str, torch.Tensor]):
        ...
        c3_top_down = c3_top_down + x['c3']

        c2_top_down = F.interpolate(c3_top_down, size=(x['c2'].shape[2], x['c2'].shape[3]), mode=_INTERP_MODE_)
        c2_top_down = c2_top_down + x['c2']

        c1_top_down = F.interpolate(c2_top_down, size=(x['c1'].shape[2], x['c1'].shape[3]), mode=_INTERP_MODE_)
        c1_top_down = c1_top_down + x['c1']

        c0_top_down = F.interpolate(c1_top_down, size=(x['c0'].shape[2], x['c0'].shape[3]), mode=_INTERP_MODE_)
        c0_top_down = c0_top_down + x['c0']

        # return a dictionary of node outputs
        x_out = {}
        x_out['c0'] = self.c0_top_down_nodes(c0_top_down)
        x_out['c1'] = self.c1_top_down_nodes(c1_top_down)
        x_out['c2'] = self.c2_top_down_nodes(c2_top_down)
        x_out['c3'] = self.c3_top_down_nodes(c3_top_down)
        x_out['c4'] = self.c4_top_down_nodes(c4_top_down)
        return x_out
