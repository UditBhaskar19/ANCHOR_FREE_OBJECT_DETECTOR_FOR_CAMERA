    ...
    def forward(self, x: Dict[str, torch.Tensor]):
        ...
        for i in range(self.num_top_down_nodes):
            ...
            x_interpolate = F.interpolate(
                ...
                mode = _INTERP_MODE_) 
            x_top_down1[key] = top_down_weights[i, 0] * x[key] + top_down_weights[i, 1] * x_interpolate
            prev_node = x_top_down1[key]

        x_top_down = {}
        for key, val in x_top_down1.items():
            x_top_down[key] = self.top_down_nodes[key](val)
        ...
        x_interpolate = F.interpolate(
                ...
                mode = _INTERP_MODE_) 
        x_bottom_up['c0'] = bottom_up_weights[0, 0] * x['c0'] + bottom_up_weights[0, 1] * x_interpolate
        ...
        for i in range(self.num_top_down_nodes):
            ...
            x_interpolate = F.interpolate(
                ...
                size = ( x[key].shape[2], x[key].shape[3] ), 
                mode = _INTERP_MODE_)
            x_bottom_up[key] = bottom_up_weights[i+1, 0] * x[key] + \
                               bottom_up_weights[i+1, 1] * x_top_down[key] + \
                               bottom_up_weights[i+1, 2] * x_interpolate
        ...
        x_interpolate = F.interpolate(
            ...
            size = ( x[key].shape[2], x[key].shape[3] ), 
            mode = _INTERP_MODE_)
        x_bottom_up[key] = bottom_up_weights[-1, 0] * x[key] + bottom_up_weights[-1, 1] * x_interpolate

        x_out = {}
        for key, val in x_bottom_up.items():
            x_out[key] = self.bottom_up_nodes[key](val)
        return x_out
