...
class BiFPN_block(nn.Module):
    ...
    def forward(self, x: Dict[str, torch.Tensor]):
        ...
        c4_top_down = self.c4_top_down_nodes( top_down_weights[0,0] * x['c4'] + top_down_weights[0,1] * c4_top_down )

        c3_top_down = F.interpolate(c4_top_down, size=(x['c3'].shape[2], x['c3'].shape[3]), mode=_INTERP_MODE_) 
        c3_top_down = self.c3_top_down_nodes( top_down_weights[1,0] * x['c3'] + top_down_weights[1,1] * c3_top_down )

        c2_top_down = F.interpolate(c3_top_down, size=(x['c2'].shape[2], x['c2'].shape[3]), mode=_INTERP_MODE_) 
        c2_top_down = self.c2_top_down_nodes( top_down_weights[2,0] * x['c2'] + top_down_weights[2,1] * c2_top_down )

        c1_top_down = F.interpolate(c2_top_down, size=(x['c1'].shape[2], x['c1'].shape[3]), mode=_INTERP_MODE_) 
        c1_top_down = self.c1_top_down_nodes( top_down_weights[3,0] * x['c1'] + top_down_weights[3,1] * c1_top_down )

        # bottom-up path
        c0_bottom_up = F.interpolate(c1_top_down, size=(x['c0'].shape[2], x['c0'].shape[3]), mode=_INTERP_MODE_)
        c0_bottom_up = self.c0_bottom_up_nodes( bottom_up_weights[5,0] * x['c0'] + bottom_up_weights[5,1] * c0_bottom_up )

        c1_bottom_up = F.interpolate(c0_bottom_up, size=(x['c1'].shape[2], x['c1'].shape[3]), mode=_INTERP_MODE_)
        c1_bottom_up = self.c1_bottom_up_nodes( bottom_up_weights[4,0] * x['c1'] + bottom_up_weights[4,1] * c1_bottom_up + bottom_up_weights[4,2] * c1_top_down )

        c2_bottom_up = F.interpolate(c1_bottom_up, size=(x['c2'].shape[2], x['c2'].shape[3]), mode=_INTERP_MODE_)
        c2_bottom_up = self.c2_bottom_up_nodes( bottom_up_weights[3,0] * x['c2'] + bottom_up_weights[3,1] * c2_bottom_up + bottom_up_weights[3,2] * c2_top_down )

        c3_bottom_up = F.interpolate(c2_bottom_up, size=(x['c3'].shape[2], x['c3'].shape[3]), mode=_INTERP_MODE_)
        c3_bottom_up = self.c3_bottom_up_nodes( bottom_up_weights[2,0] * x['c3'] + bottom_up_weights[2,1] * c3_bottom_up + bottom_up_weights[2,2] * c3_top_down )

        c4_bottom_up = F.interpolate(c3_bottom_up, size=(x['c4'].shape[2], x['c4'].shape[3]), mode=_INTERP_MODE_)
        c4_bottom_up = self.c4_bottom_up_nodes( bottom_up_weights[1,0] * x['c4'] + bottom_up_weights[1,1] * c4_bottom_up + bottom_up_weights[1,2] * c4_top_down )

        c5_bottom_up = F.interpolate(c4_bottom_up, size=(x['c5'].shape[2], x['c5'].shape[3]), mode=_INTERP_MODE_)
        c5_bottom_up = self.c5_bottom_up_nodes( bottom_up_weights[0,0] * x['c5'] + bottom_up_weights[0,1] * c5_bottom_up )
