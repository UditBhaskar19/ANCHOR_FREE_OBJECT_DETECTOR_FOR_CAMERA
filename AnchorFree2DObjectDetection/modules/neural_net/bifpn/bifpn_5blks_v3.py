...
class BiFPN_block(nn.Module):
    ...
    def forward(self, x: Dict[str, torch.Tensor]):
        ...
        c3_top_down = top_down_weights[0,0] * x['c3'] + top_down_weights[0,1] * c3_top_down

        c2_top_down = F.interpolate(c3_top_down, size=(x['c2'].shape[2], x['c2'].shape[3]), mode=_INTERP_MODE_) 
        c2_top_down = top_down_weights[1,0] * x['c2'] + top_down_weights[1,1] * c2_top_down

        c1_top_down = F.interpolate(c2_top_down, size=(x['c1'].shape[2], x['c1'].shape[3]), mode=_INTERP_MODE_) 
        c1_top_down = top_down_weights[2,0] * x['c1'] + top_down_weights[2,1] * c1_top_down
        ...
        c0_bottom_up = bottom_up_weights[4,0] * x['c0'] + bottom_up_weights[4,1] * c0_bottom_up

        c1_bottom_up = F.interpolate(c0_bottom_up, size=(x['c1'].shape[2], x['c1'].shape[3]), mode=_INTERP_MODE_)
        c1_bottom_up = bottom_up_weights[3,0] * x['c1'] + bottom_up_weights[3,1] * c1_bottom_up + bottom_up_weights[4,2] * c1_top_down

        c2_bottom_up = F.interpolate(c1_bottom_up, size=(x['c2'].shape[2], x['c2'].shape[3]), mode=_INTERP_MODE_)
        c2_bottom_up = bottom_up_weights[2,0] * x['c2'] + bottom_up_weights[2,1] * c2_bottom_up + bottom_up_weights[3,2] * c2_top_down

        c3_bottom_up = F.interpolate(c2_bottom_up, size=(x['c3'].shape[2], x['c3'].shape[3]), mode=_INTERP_MODE_)
        c3_bottom_up = bottom_up_weights[1,0] * x['c3'] + bottom_up_weights[1,1] * c3_bottom_up + bottom_up_weights[2,2] * c3_top_down

        c4_bottom_up = F.interpolate(c3_bottom_up, size=(x['c4'].shape[2], x['c4'].shape[3]), mode=_INTERP_MODE_)
        c4_bottom_up = bottom_up_weights[0,0] * x['c4'] + bottom_up_weights[0,1] * c4_bottom_up
