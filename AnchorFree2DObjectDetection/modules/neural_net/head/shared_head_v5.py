...
from modules.neural_net.constants import (
    _NUM_GROUPS_, _INTERP_MODE_, det_named_tuple,
...
class MergeFeatures(nn.Module):
    ...
    def forward(self, x: Dict[str, torch.Tensor]) -> torch.Tensor:
        # upsample the feature maps to dimension ( out_feat_h, out_feat_w )
        upsampled_maps = []
...
class SharedNet(nn.Module):
    def __init__(
        ...
        stem_channels_cls = net_config_obj.stem_channels
        stem_channels_reg = net_config_obj.stem_channels
        ...
        # stem blocks
        self.stem_blk_cls = StemBlock(
                in_channels=in_channels, 
                stem_channels=stem_channels_cls,
                activation=activation)
        
        self.stem_blk_reg = StemBlock(
                in_channels=in_channels, 
                stem_channels=stem_channels_reg,
                activation=activation)

        # Class prediction head
        self.pred_cls = TaskSpecificHead(
                            in_channels=stem_channels_cls[-1],
        ...
        self.pred_box = TaskSpecificHead(
                            in_channels=stem_channels_reg[-1],
                            out_channels=4,
                            activation=activation,
    ...
    def forward(self, x: Dict[str, torch.Tensor]):
        ...
        featmap_cls = self.stem_blk_cls(featmap)
        featmap_reg = self.stem_blk_reg(featmap)                 

        # task specific predictions
        class_logits = self.pred_cls(featmap_cls)               # Class prediction
        objness_logits = self.pred_obj(featmap_cls)             # Objectness prediction
        centerness_logits = self.pred_ctr(featmap_reg)          # Centerness regression
        boxreg_deltas = self.pred_box(featmap_reg)              # Box regression
