...
from modules.augmentation import constants_bdd
...
class Augment():
    def __init__(self, img_w: int, img_h: int, prob_augment: float):
        ...
        self.prob_geometric = constants_bdd._PROB_GEOMETRIC_ 
        self.prob_mosaic = constants_bdd._PROB_MOSAIC_ 
        self.prob_mixup = constants_bdd._PROB_MIXUP_
        self.prob_flip = constants_bdd._PROB_FLIP_
        self.prob_crop = constants_bdd._PROB_CROP_

        self.hgain = constants_bdd._H_GAIN_
        self.sgain = constants_bdd._S_GAIN_
        self.vgain = constants_bdd._V_GAIN_

        self.degree = constants_bdd._MAX_DEGREE_
        self.translate = constants_bdd._MAX_TRANSLATE_
        self.scale = constants_bdd._MAX_SCALE_
        self.shear = constants_bdd._MAX_SHEAR_
        self.perspective = constants_bdd._PERSPECTIVE_

        self.mixup = mixup(alpha=constants_bdd._MIXUP_ALPHA_, beta=constants_bdd._MIXUP_BETA_)
        ...
        self.pixel_dropout = pixel_dropout(
            num_min=constants_bdd._MIN_PIXEL_DROPOUT_, num_max=constants_bdd._MAX_PIXEL_DROPOUT_)
        self.block_dropout = block_dropout(
            num_min=constants_bdd._MIN_BLOCK_DROPOUT_, num_max=constants_bdd._MAX_BLOCK_DROPOUT_, 
            h_min=constants_bdd._MIN_BLOCK_H_, w_min=constants_bdd._MIN_BLOCK_W_, 
            h_max=constants_bdd._MAX_BLOCK_H_, w_max=constants_bdd._MAX_BLOCK_W_)
        self.grid_dropout = grid_dropout(
            max_start_offset=constants_bdd._MAX_START_OFFSET_GRID_DROPOUT_, 
            h_min=constants_bdd._MIN_CELL_H_, w_min=constants_bdd._MIN_CELL_W_, 
            h_max=constants_bdd._MAX_CELL_H_, w_max=constants_bdd._MAX_CELL_W_,
            num_row_min=constants_bdd._MIN_GRID_ROWS_, num_row_max=constants_bdd._MAX_GRID_ROWS_, 
            num_col_min=constants_bdd._MIN_GRID_COLS_, num_col_max=constants_bdd._MAX_GRID_COLS_)
    ...
    def resize_image_bbox(
        ...
        image = cv2.resize(image, (self.img_w, self.img_h), interpolation=constants_bdd._INTERPOLATION_MODE_)
        return image, bbox
    ...
    def extract_img_box_label(
        ...
        img = np.array(Image.open(dataset_gen.selected_labels[idx]['img_path']).convert("RGB"))
        bbox = dataset_gen.selected_labels[idx]['boundingBox2D'].copy()
        if bbox.shape[0] == 0: bbox = np.zeros(shape=(0, 4), dtype=np.float32)
        img, bbox = self.resize_image_bbox(img, bbox)
        classlabels = dataset_gen.selected_labels[idx]['objCategoryid'].copy()
    ...
    if np.random.rand() <= aug_obj.prob_augment:             # perform augmentation
        ...
        if np.random.rand() <= aug_obj.prob_mosaic:          # perform 2 x 2 mosaic augmentation
            ...
            if np.random.rand() <= aug_obj.prob_crop:
                scale = np.random.uniform(low=constants_bdd._SCALED_CROP_MIN_, high=constants_bdd._SCALED_CROP_MAX_)  
        ...
        if np.random.rand() <= aug_obj.prob_geometric:                   # geometric
            img, T, height, width = random_perspective(                      
                ...
                aug_obj.shear, aug_obj.perspective, border = constants_bdd._BORDER_)
