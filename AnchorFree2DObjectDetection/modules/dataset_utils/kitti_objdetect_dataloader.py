# ---------------------------------------------------------------------------------------------------------------------
# Author : Udit Bhaskar
# Description : kitti dataset and dataloader functions for pytorch based training sample generation
# ---------------------------------------------------------------------------------------------------------------------
import numpy as np
...
from modules.augmentation.augment_kitti import Augment
...
# ---------------------------------------------------------------------------------------------------------------------
class KittiTrackingDataset_2DObjectDetection(torch.utils.data.Dataset):
    def __init__(
        self,
        gt_labels,
    ...
    def extract_img_box_label(self, idx: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray, str, int]:
        img_path = self.selected_labels[idx]['image_path']
        img = np.array(Image.open(img_path).convert("RGB"))
        bbox = self.selected_labels[idx]['bbox'].copy()
        if bbox.shape[0] == 0: bbox = np.zeros(shape=(0, 4), dtype=np.float32)
        img, bbox = self.resize_image_bbox_np(img, bbox)
        classlabels = self.selected_labels[idx]['classid'].copy()
        return img, bbox, classlabels, img_path
    ...
    def __getitem__(self, idx: int) \
        ...
        if self.augment: 
            ...
            if is_augmented : img_path = 'Augmented Image'
            else: img_path = self.selected_labels[idx]['image_path']    
        else: img, bbox, classlabels, img_path = self.extract_img_box_label(idx)
        ...
        if num_objects > 0:
            bbox, classlabels = extract_valid_bbox(bbox, classlabels)
            labels['bbox'] = torch.Tensor(bbox)
            labels['obj_class_label'] = torch.Tensor(classlabels)
        else:
